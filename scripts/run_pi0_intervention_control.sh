...
if ! PYTHONPATH="$repo_root/src" "$repo_root/.venv/bin/python" -c \
  'import json,re,sys; value=json.load(open(sys.argv[1])); hashes=value.get("pytorch_checkpoint_artifact_sha256", {}); identity=value.get("jax_checkpoint_identity", {}); assert value.get("passed") is True; assert int(value.get("cases", -1)) == 32 and int(value.get("passed_cases", -1)) == 32; assert value.get("shape_per_case") == [50, 7]; assert float(value.get("max_abs_tolerance", -1)) == 0.02; assert float(value.get("minimum_cosine_similarity", -1)) == 0.999; assert set(hashes) == {"config.json", "model.safetensors"}; assert all(re.fullmatch(r"[0-9a-f]{64}", str(item)) for item in hashes.values()); assert identity.get("finalized") is True and int(identity.get("optimizer_updates", -1)) == 30000' \
  "$parity_summary"; then
