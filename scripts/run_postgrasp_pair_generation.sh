...
if [[ $# -lt 5 || $# -gt 7 ]]; then
  echo "usage: $0 <manifest> <pair-id> <gpu> <rollout-dir> <output-dir> [lift-threshold-m] [persistence-steps]" >&2
  exit 2
fi
...
lift_threshold="${6:-0.02}"
persistence_steps="${7:-5}"
...
"${docker_command[@]}" run --rm \
  ...
  /bin/bash -lc \
  "source /.venv/bin/activate && python /app/scripts/generate_postgrasp_instruction_pairs.py \
    --source-manifest /pair/$(basename "$manifest") \
    --pair-id '$pair_id' \
    --rollout /rollout \
    --output /data \
    --lift-threshold '$lift_threshold' \
    --persistence-steps '$persistence_steps'"
